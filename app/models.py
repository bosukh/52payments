...
class User(ndb.Model):
    ...
    email_validated = ndb.BooleanProperty(default=False, indexed=True)
    phone = ndb.StringProperty(required=False, indexed=False)
    phone_validated = ndb.BooleanProperty(default=False, indexed=True)
    password = ndb.StringProperty(required=False, indexed=False)
    authenticated = ndb.BooleanProperty(default=False, indexed=False)
    company_admin = ndb.StringProperty(required=False, indexed=True)
    created = ndb.DateTimeProperty(auto_now_add = True, indexed=False)
    last_modified = ndb.DateTimeProperty(auto_now = True, indexed=False)
